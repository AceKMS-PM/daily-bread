...
function PasswordRule({ ok, label }: { ok: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2 text-xs font-sans">
      {ok ? (
        <Check size={12} style={{ color: "#A4B478" }} />
      ) : (
        <X size={12} style={{ color: "rgba(249,241,224,0.3)" }} />
      )}
...
export default function SignInPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Validation frontend avant d'appeler le backend
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              {/* Règles mot de passe — visibles à l'inscription */}
              {isSignUp && (passwordFocused || password.length > 0) && (
                <div
                  className="mt-3 p-3 rounded-lg flex flex-col gap-1.5"
            ...
            {error && (
              <div
                className="px-4 py-3 rounded-lg font-sans text-sm text-center"
                style={{ background: "rgba(139,32,32,0.2)", border: "1px solid rgba(139,32,32,0.4)", color: "#f87171" }}
              >
                {error}
              </div>
            )}
            ...
            <button
              ...
              style={{ opacity: loading || !formValid ? 0.5 : 1, cursor: !formValid ? "not-allowed" : "pointer" }}
            >
